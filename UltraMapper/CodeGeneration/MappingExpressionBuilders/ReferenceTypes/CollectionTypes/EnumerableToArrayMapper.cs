...
namespace UltraMapper.MappingExpressionBuilders
{
    ...
    /// </summary>
    public class EnumerableToArrayMapper : CollectionMappingViaTempCollection
    {
        ...
        private readonly static MethodInfo _toArrayMethod =
            typeof( List<> ).GetMethod( nameof( List<int>.ToArray ) );

        protected override Expression GetExpressionBody( ReferenceMapperContext contextObj )
        {
            var collContext = contextObj as CollectionMapperContext;

            var exp = Expression.Block
            (
                Expression.Assign( contextObj.TargetInstance,
                    Expression.Call( _toArrayMethod, base.GetExpressionBody( contextObj ) ) )
            );
